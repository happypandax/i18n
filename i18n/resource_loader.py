...
def get_namespace_from_filepath(filename):
    ...
    if '{namespace}' in config.get('filename_format'):
        try:
            ...
            namespace += splitted_filename[config.get('filename_format').split(config.get('namespace_delimiter')).index('{namespace}')]
        except ValueError:
...
def load_translation_dic(dic, namespace, locale):
    ...
    for key, value in dic.items():
        if type(value) == dict and config.get('namespace_in_file'):
